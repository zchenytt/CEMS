const (K, F, Krho, my_seed) = (3, 1, 3, 0)
println(*(
    "$PROGRAM_FILE> (K, F, Krho, my_seed) = $((K, F, Krho, my_seed)), ",
    "nthreads = $((Threads.nthreads(), Threads.nthreads(:interactive))), ",
    "at id = $(Threads.threadid()) in $(Threads.threadpool())"
))

# To showcase the effectiveness of the EV lending, we ban the ES module temporarily (given BESS in reality is relatively expensive)
# 1. with the EV-lending function, the feasibility condition is easier to meet (e.g. houses in a paired block can have smaller circuit breaker)
# 2.The resulting figure indicates that The demand response is more responsive.

import Random; Random.seed!(hash(my_seed));
import Gurobi; const GRB_ENV = Gurobi.Env();
import JuMP, Statistics, JLD2
get_simple_model() = (m = JuMP.direct_model(Gurobi.Optimizer(GRB_ENV)); JuMP.set_silent(m); JuMP.set_attribute(m, "Threads", 1); JuMP.set_objective_sense(m, JuMP.MIN_SENSE); m);
ref!(X, m, x) = (o = m.moi_backend; Gurobi.GRBgetdblattrelement(o, "X", Gurobi.c_column(o, JuMP.index(x)), X));
value!(X, m, x) = foreach(j -> ref!(view(X, j), m, x[j]), eachindex(X));
...
ˈ96ˈ24(t) = (t-1)÷F+1;
abstract type Vertex end; # need to export (Public || Integer) variables within each block
...
fillvertex!(v::Vertex, j, X) = ((m, x) = (inn[j], X[j]); foreach(s -> value!(getproperty(v, s), m, getproperty(x, s)), propertynames(v)));
...
function gen_ac_data()::Tuple
    ...
    COP   = rand(2:.5:4)
    Q_I   = rand(3:9, T)
    Q_BUS = rand(25:35) # generate, but may not set it at first 
    OH    = rand(24:29) 
    OΔ    = rand(4:9)
    P_AC  = pc_P_AC(O, OH, CND, Q_I, COP)
    CND, INR, COP, Q_I, Q_BUS, OH, OΔ, P_AC
end;
function add_AC_module!(model, O, CND, INR, COP, Q_I, OH, OΔ, P_AC; Q_BUS)
    pAC = JuMP.@variable(model, [1:T], lower_bound = 0, upper_bound = P_AC)
    o = JuMP.@variable(model, [1:T], lower_bound = OH-OΔ, upper_bound = OH)
    q = JuMP.@variable(model, [1:T], lower_bound = 0, upper_bound = Q_BUS) # Here
...
function pc_self_P_BUS(D, U, P_EV, E_EV, O, CND, INR, COP, Q_I, OH, OΔ, P_AC)::Tuple{Bool, Int64}
    ...
    o, q, pAC = add_AC_module!(model, O, CND, INR, COP, Q_I, OH, OΔ, P_AC; Q_BUS = 0)
...
function add_self_circuit_breaker_module!(model, P_BUS, D, pU, pEV, pAC)
    ...
    JuMP.@constraint(model, pBus .≥ D + pU + pEV + pAC) # No G | ES
    pBus # paired with Cmeaned in terms of cost
end;
function add_circuit_breaker_pair_module!(model, P_BUS_1, P_BUS_2, pES, G, pLent, pEV_1, pU_1, pAC_1, D_1, pEV_2, pU_2, pAC_2, D_2; T = T, F = F)
    ...
    f = ˈ96ˈ24
    # JuMP.@constraint(model, [t = 1:(F)T], pBus_1[t] == -G[t] + pLent[f(t)] + pEV_1[f(t)] + pU_1[f(t)] + pAC_1[f(t)] + D_1[f(t)])
    JuMP.@constraint(model, [t = 1:(F)T], pBus_1[t] == pES.c[t] -pES.d[t] -G[t] + pLent[f(t)] + pEV_1[f(t)] + pU_1[f(t)] + pAC_1[f(t)] + D_1[f(t)])
    ...
    pBus, pBus_1, pBus_2 # pBus is paired with C/F in terms of cost
end;
function add_ES_module!(model, P_ES, E_ES)
    ...
    eES = JuMP.@variable(model, [t=1:(F)T], lower_bound = ifelse(t<(F)T, 0, E_ES.e), upper_bound = E_ES.M)
...
function get_a_paired_block(O)::NamedTuple
    while true
        ...
        P_EV_1, E_EV_1 = (m = rand((1., 1.5)), M = rand(5:8)), rand(20:90)
        ...
        JuMP.fix.(bLent, 1)
        o_1, q_1, pAC_1 = add_AC_module!(model, O, CND_1, INR_1, COP_1, Q_I_1, OH_1, OΔ_1, P_AC_1; Q_BUS = 0) 
        # 4 lines
        ...
        P_EV_2, E_EV_2 = (m = rand((1., 1.5)), M = rand(4:7)), rand(72:96)
        ...
        o_2, q_2, pAC_2 = add_AC_module!(model, O, CND_2, INR_2, COP_2, Q_I_2, OH_2, OΔ_2, P_AC_2; Q_BUS = 0)
        ...
        JuMP.optimize!(model)
        ps, ts = JuMP.primal_status(model), JuMP.termination_status(model)
        ps === JuMP.FEASIBLE_POINT || (@ccall(printf("get_a_paired_block> re-generating...\n"::Cstring)::Cint); continue)
        (ts === JuMP.OPTIMAL || ts === JuMP.TIME_LIMIT) || (@ccall(printf("get_a_paired_block> re-generating...\n"::Cstring)::Cint); continue)
        temp_float64 = JuMP.value(temp_x)
        temp_float64 > 0 || (@ccall(printf("get_a_paired_block> re-generating...\n"::Cstring)::Cint); continue)
        ...
        f = ˈ96ˈ24
        # JuMP.@constraint(model, [t = 1:(F)T], -temp_x ≤ -G[t] + pLent[f(t)] + pEV_1[f(t)] + pU_1[f(t)] + pAC_1[f(t)] + D_1[f(t)])
        # JuMP.@constraint(model, [t = 1:(F)T],  temp_x ≥ -G[t] + pLent[f(t)] + pEV_1[f(t)] + pU_1[f(t)] + pAC_1[f(t)] + D_1[f(t)])
        ...
        (ts === JuMP.OPTIMAL || ts === JuMP.TIME_LIMIT) || (@ccall(printf("get_a_paired_block> re-generating...\n"::Cstring)::Cint); continue)
        temp_float64 = JuMP.value(temp_x)
        temp_float64 > -1e-5 || (@ccall(printf("get_a_paired_block> re-generating...\n"::Cstring)::Cint); continue)
...
function get_a_self_block(O)::NamedTuple
    while true
        ...
        (@ccall(printf("get_a_self_block> re-generating...\n"::Cstring)::Cint); continue)
    end
end;
function add_a_paired_block!(model, d::NamedTuple; no_cool)::NamedTuple
    # lender house
    ...
    o_1, q_1, pAC_1 = add_AC_module!(model, O, d.CND_1, d.INR_1, d.COP_1, d.Q_I_1, d.OH_1, d.OΔ_1, d.P_AC_1; Q_BUS = ifelse(no_cool, 0, d.Q_BUS_1))
    # borrower house
    bU_2, pU_2 = add_U_module!(model, d.U_2)
    bEV_2, pEV_2 = add_EV_2_module!(model, d.P_EV_2, d.E_EV_2, bLent, pLent)
    o_2, q_2, pAC_2 = add_AC_module!(model, O, d.CND_2, d.INR_2, d.COP_2, d.Q_I_2, d.OH_2, d.OΔ_2, d.P_AC_2; Q_BUS = ifelse(no_cool, 0, d.Q_BUS_2))
    ...
    # export the `q` variable
    q = JuMP.@variable(model, [1:T])
    JuMP.@constraint(model, q .== q_1 + q_2)
    (;pBus, pBus_1, pBus_2, bLent, bES, bEV_1, bEV_2, bU_1, bU_2, q_1, q_2, q)
end;
function add_a_self_block!(model, d::NamedTuple; no_cool)::NamedTuple
    bU, pU = add_U_module!(model, d.U)
    bEV, pEV = add_self_EV_module!(model, d.P_EV, d.E_EV)
    o, q, pAC = add_AC_module!(model, O, d.CND, d.INR, d.COP, d.Q_I, d.OH, d.OΔ, d.P_AC; Q_BUS = ifelse(no_cool, 0, d.Q_BUS))
    pBus = add_self_circuit_breaker_module!(model, d.P_BUS, d.D, pU, pEV, pAC)
    (;pBus, bEV, bU, q)
end;
function solve_mst_and_up_value!(m, s)
    JuMP.optimize!(m)
    ...
    s.common.x = JuMP.value(m[:common])
    s.ν.x = JuMP.value(ν)
    value!(s.μ, m, μ)
    value!(s.β, m, β)
    value!(s.θ, m, θ)
...
function shot!(ref)
    s = rEF.x
    @lock mst_lock solve_mst_and_up_value!(model, s) # `s` gets updated/mutated here
    s_tmp = ref.x
    setfield!(ref, :x, s) # the ref gets updated here
    setfield!(rEF, :x, s_tmp)
    nothing
end;
function initialize_out(P_A, Q_A, E_A)
    model = get_simple_model()
    JuMP.@variable(model, β[1:(F)T] ≥ 0)
    JuMP.@variable(model, μ[1:T] ≥ 0)
    JuMP.@variable(model, ν ≥ 0)
    JuMP.@variable(model, θ[1:J])
    JuMP.@expression(model, common, -sum(β)P_A -sum(μ)Q_A -(ν)E_A)
    JuMP.@objective(model, Max, common + sum(θ))
    model, θ, β, μ, ν
end;
function bilin_expr(j, iˈı::Function, β, μ, ν) # ⚠️ the value of ν VS the JuMPvariable ν are NOT the same
    h = ifelse(j in Rng1, identity, ˈ96ˈ24)
    p, q = X[j].pBus, X[j].q
    e = (
        JuMP.@expression(model, sum( iˈı(p[ h(t) ])β[ t ] for t = 1:(F)T)),
        JuMP.@expression(model, sum( iˈı(q[t])μ[t] for t = 1:T))
    )
    JuMP.@expression(model, iˈı(inn[j][:primobj]) + sum(e) + sum(iˈı, q)ν)
end;
function subproblemˈs_duty(j; initialize = false)
    mj = inn[j]
    while true
        s = getfield(ref, :x)
        exp = bilin_expr(j, identity, s.β, s.μ, s.ν.x)
        foreach(e -> JuMP.set_objective_coefficient(mj, e, JuMP.coefficient(exp, e)), X[j].pBus)
        foreach(e -> JuMP.set_objective_coefficient(mj, e, JuMP.coefficient(exp, e)), X[j].q)
        JuMP.optimize!(mj)
        ...
        if ps !== JuMP.FEASIBLE_POINT || (ts !== JuMP.OPTIMAL && ts !== JuMP.TIME_LIMIT)
            ...
            @ccall(printf("subproblemˈs_duty> re-generating...\n"::Cstring)::Cint)
            continue
        end
        if !initialize
            s = getfield(ref, :x)
            s.θ[j] - bilin_expr(j, JuMP.value, s.β, s.μ, s.ν.x) > COT || return
        end
        ...
        con = JuMP.@build_constraint(θ[j] ≤ bilin_expr(j, JuMP.value, β, μ, ν))
...
function subproblemˈs_objbnd!(pvv, j)
    ...
    while true
        ...
        @ccall(printf("subproblemˈs_objbnd!> re-solving...\n"::Cstring)::Cint)
    end
end;
next(j, J) = j === J ? 1 : j+1;
...
function main(train_time, mst_task_ref, tasks)
    tdue = time_ns() + 1e9 * train_time
    Ncuts0 = Ncuts.value
    j = 1
    while proceed_main.x && time_ns() < tdue # Run with a time limit, or you manually interrupt
        ...
        while ntasksaway(tasks) < THREAD_FOR_BLOCKS  # for block subproblems
            istaskdone(tasks[j]) && setindex!(tasks, Threads.@spawn(subproblemˈs_duty($j)), j)
            j = next(j, J)
        end
        GC.safepoint()
    end
...
function main(train_time)
    mst_task_ref = Ref{Task}(Threads.@spawn(shot!(ref)))
    tasks = Task[Threads.@spawn(missing) for _ = 1:J]
...
function primal_recovery(model) 
    ...
    Y = [j ∈ Rng1 ? (
        bES = @get_int_decision(model, sum(map(round, t.bES)l for (l, t) = zip(l[j], VCG[j]))),
        bU_1 = @get_int_decision(model, sum(map(round, t.bU_1)l for (l, t) = zip(l[j], VCG[j]))),
        bU_2 = @get_int_decision(model, sum(map(round, t.bU_2)l for (l, t) = zip(l[j], VCG[j]))),
        bEV_1 = @get_int_decision(model, sum(map(round, t.bEV_1)l for (l, t) = zip(l[j], VCG[j]))),
        bEV_2 = @get_int_decision(model, sum(map(round, t.bEV_2)l for (l, t) = zip(l[j], VCG[j]))),
        bLent = @get_int_decision(model, sum(map(round, t.bLent)l for (l, t) = zip(l[j], VCG[j]))),
        pBus = JuMP.@expression(model, sum((t.pBus)l for (l, t) = zip(l[j], VCG[j]))), # of length (F)T
        q = JuMP.@expression(model, sum((t.q)l for (l, t) = zip(l[j], VCG[j])))
    ) : (
        bU = @get_int_decision(model, sum(map(round, t.bU)l for (l, t) = zip(l[j], VCG[j]))),
        bEV = @get_int_decision(model, sum(map(round, t.bEV)l for (l, t) = zip(l[j], VCG[j]))),
        pBus = JuMP.@expression(model, sum((t.pBus)l for (l, t) = zip(l[j], VCG[j]))), # of length T
        q = JuMP.@expression(model, sum((t.q)l for (l, t) = zip(l[j], VCG[j])))
    ) for j = 1:J]
    p_A = [zero(JuMP.AffExpr) for t = 1:(F)T]
    q_A = [zero(JuMP.AffExpr)    for t = 1:T]
    e_A = zero(JuMP.AffExpr)
    for j = 1:J
        p, q = Y[j].pBus, Y[j].q
        f = ifelse(j in Rng1, identity, ˈ96ˈ24)
        for t = 1:(F)T
            JuMP.add_to_expression!(p_A[t], p[f(t)])
        end
        for t = 1:T
            JuMP.add_to_expression!(q_A[t], q[t])
            JuMP.add_to_expression!(e_A, q[t])
        end
    end
    # maximum(sum(VCG[j][1].pBus[ifelse(j in Rng1, t, ˈ96ˈ24(t))] for j = 1:J) for t = 1:(F)T)
    # maximum(sum(VCG[j][1].q[t] for j = 1:J) for t = 1:T)
    # sum(sum(VCG[j][1].q[t] for j = 1:J) for t = 1:T)
    JuMP.@constraint(model, P_A .≥ p_A) # beta
    JuMP.@constraint(model, Q_A .≥ q_A) # mu
    JuMP.@constraint(model, E_A  ≥ e_A) # nu
    JuMP.@objective(model, Min, 1/F * C'p_A)
    JuMP.optimize!(model)
    @ccall(printf("primal_recovery> done.\n"::Cstring)::Cint)
    ps, ts = JuMP.primal_status(model), JuMP.termination_status(model)
    (ps === JuMP.FEASIBLE_POINT && (ts === JuMP.OPTIMAL || ts === JuMP.TIME_LIMIT)) || error("primal_recovery: $ps, $ts")
    JuMP.solve_time(model), JuMP.objective_value(model) 
end;

initialize_ref() = Ref((θ = fill(an_UB/J, J), β = fill(0., (F)T), μ = fill(0., T), ν = Ref(0.), ub = Ref(1.0 * an_UB), common = Ref(0.)));
const (Rng1, Rng2), (C, O) = get_pair_and_self_Rng(J), get_C_and_O();
const Cmeaned = F === 4 ? map(t -> Statistics.mean(view(C, 4(t-1)+1:4t)), 1:T) : C # used for self houses to do dot product
const MaxVerNum, proceed_main = 150, Ref(true); # increase this if not sufficient
const COT, an_UB = 100/J, 19999.9J; # TODO To adapt
const mst_lock, selfblockrgap_lock, pairblockrgap_lock = ReentrantLock(), ReentrantLock(), ReentrantLock()
const inn, Ncuts = [get_simple_model() for j = 1:J], Threads.Atomic{Int}(0);
const ref, rEF = initialize_ref(), initialize_ref();
const D = Vector{NamedTuple}(undef, J); # the central raw data
const X = Vector{NamedTuple}(undef, J); # X is inn's
function fillmodels!(j)
    c, mj = j ∈ Rng1, inn[j]
    D[j] = Dj = ifelse(c, get_a_paired_block, get_a_self_block)(O) # raw data
    X[j] = Xj = ifelse(c, add_a_paired_block!, add_a_self_block!)(mj, Dj; no_cool = true)
    JuMP.@expression(mj, primobj, ifelse(c, C/F, Cmeaned)'Xj.pBus)
    JuMP.set_objective_function(mj, primobj)
    JuMP.set_attribute(mj, "TimeLimit", 300)
    JuMP.optimize!(mj)
    ps, ts = JuMP.primal_status(mj), JuMP.termination_status(mj)
    (ps === JuMP.FEASIBLE_POINT && (ts === JuMP.OPTIMAL || ts === JuMP.TIME_LIMIT)) || error(ts)
end;
foreach(wait, [Threads.@spawn(fillmodels!(j)) for j = 1:J])

const cen = get_simple_model();
const Xc = Vector{NamedTuple}(undef, J);
function build_cen!()
    pA = [zero(JuMP.AffExpr) for _ = 1:(F)T]
    qA = [zero(JuMP.AffExpr)    for _ = 1:T]
    eA = zero(JuMP.AffExpr)
    for j = 1:J
        c = j in Rng1
        Xc[j] = Xj = ifelse(c, add_a_paired_block!, add_a_self_block!)(cen, D[j]; no_cool = true)
        h = ifelse(c, identity, ˈ96ˈ24)
        for t = 1:(F)T
            JuMP.add_to_expression!(pA[t], Xj.pBus[h(t)])
            JuMP.add_to_expression!(qA[t], Xj.q[t])
            JuMP.add_to_expression!(eA,    Xj.q[t])
        end
    end
    # JuMP.@constraint(cen, 5.5 * 13484 .>= pA) # this stage is infeasible if EV Lending is turned off
    JuMP.@constraint(cen, 13484 * 1.3 .>= pA) # this stage is infeasible if EV Lending is turned off
    JuMP.@constraint(cen, 10000 .>= qA)
    JuMP.@constraint(cen, 99999  >= eA)
    JuMP.@objective(cen, Min, 1/F * C'pA)
    pA, qA, eA
end;
pA, qA, eA = build_cen!();
if false # turn off EV lending
    foreach(j -> foreach(b -> JuMP.fix(b, 1), Xc[j].bLent), Rng1)
end
...
using CairoMakie
f = Figure();
ax = Axis(f[1,1]; ylabel = "Aggregated Electric Power", title = "Dispatch Results of Electric (p) and Cooling (q) Power");
ax4 = Axis(f[1,1]; ylabel = "Aggregated Cooling Power", yticklabelcolor = :blue, yaxisposition = :right);
ax2 = Axis(f[2,1]; xlabel = "Time", ylabel = "Temperature", title = "Environment Condition")
ax3 = Axis(f[2,1]; yticklabelcolor = :blue, yaxisposition = :right, ylabel = "Electric Price")
stairs!(ax2, 1:T, O; label = "temperature", color = :tomato);
stairs!(ax3, 1:T, C; label = "price", color = :blue, linestyle = :dash);
stairs!(ax4, 1:T, JuMP.value.(qA); label = "base:q", color = :blue, linestyle = :dash)
stairs!(ax, 1:T, nv; color = :seagreen, label = "base:p");
stairs!(ax, 1:T, nov; color = :tomato, label = "q=0:p");
hidespines!(ax3)
