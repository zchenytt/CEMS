...
function build_cen!()
    ...
    for j = 1:J
        c = j in Rng1
        Xc[j] = Xj = ifelse(c, add_a_paired_block!, add_a_self_block!)(cen, D[j]; no_cool = false)
    ...
    JuMP.@constraint(cen, 13484 .>= pA) # this stage is infeasible if EV Lending is turned off
...
JuMP.value.(sum(Xc[j].pBus for j = Rng1)) # retrieve this curve at turn_off = true/false, respectively

using CairoMakie
f = Figure();
ax = Axis(f[1,1]; xlabel = "Time", ylabel = "Aggregated Electric Power");
ax2 = Axis(f[1,1]; yticklabelcolor = :blue, yaxisposition = :right, ylabel = "Electric Price");
stairs!(ax2, 1:T, C; label = "price", color = :blue);
stairs!(ax, 1:T, nv; color = :seagreen, label = "base");
stairs!(ax, 1:T, nov; color = :tomato, label = "EVlent=0");
hidespines!(ax2)
