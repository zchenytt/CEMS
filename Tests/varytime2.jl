...
# end
# for F = (1, 4), S = 0:9
#     display(filter(x -> occursin("S$S", x) && occursin("F$F", x), readdir()))
# end
println(*(
...
read_value_function(s) = ifelse(s ∈ (:bES, :bLent, :bEV, :bEV_1, :bEV_2, :bU, :bU_1, :bU_2), round, identity);
...
binary_propertynames(v::Vertex) = map(Symbol, filter(startswith("b"), map(string, propertynames(v))));
...
const P_A = 1.1 * maximum(sum(JuMP.value(X[j].pBus[ifelse(j ∈ Rng1, identity, ˈ96ˈ24)(t)]) for j = 1:J) for t = 1:T);
const Q_A = 19 * J * rand(0.1:0.1:0.4);
const E_A = 19 * J * T * rand(0.1:0.1:0.4);
const model, θ, β, μ, ν = initialize_out(P_A, Q_A, E_A);
...
foreach(wait, [Threads.@spawn(subproblemˈs_duty(j; initialize = true)) for j = 1:J])
Ncuts.value === J || error(); # each block contributes 1 cut to make the master have a finite initial dual bound
shot!(ref)
function turn_on_cooling()
    for j = 1:J
        if j in Rng1
            foreach(JuMP.set_upper_bound, X[j].q_1, D[j].Q_BUS_1)
            foreach(JuMP.set_upper_bound, X[j].q_2, D[j].Q_BUS_2)
        else
            foreach(JuMP.set_upper_bound, X[j].q, D[j].Q_BUS)
        end
    end
end;
turn_on_cooling()
...
for (train_time, cg_time) = ((0, 0), (300,300), (600,900), (1800,2700))
    ...
    JuMP.set_silent(prm)
    JuMP.set_attribute(prm, "TimeLimit", max(300, cg_time/8))
