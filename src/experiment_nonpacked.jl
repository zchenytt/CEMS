...
# this should have a K axis
# TODO
# needs to do a "real-world 1-to-1" experiment time and cut with respect to solving time
# TODO [Statistics to record]
# 1. The centralized model's scale (num of variables, num of constrs...) vs the prm_recovery model's statistics
# 2. boxplot statistics of the K_VCG::Vector, with the increase of K
# Note that if we involve the mean cut, the total cut would become redundant
# 3. The mstsolvetime (median, mean, max) with the increase of K (This should have some correlation to the num of cuts finally)
# 4. block relative_gaps (median, mean, max)
# 3. Solving time (central, DW-CG, prm_recovery)
...
Statistics.mean(mstsolvetime), extrema(mstsolvetime), length(mstsolvetime)
Statistics.mean(selfblockrgap), extrema(selfblockrgap), length(selfblockrgap)
Statistics.mean(pairblockrgap), extrema(pairblockrgap), length(pairblockrgap)
