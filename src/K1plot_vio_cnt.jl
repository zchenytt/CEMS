...
warm_up()
const pvec = Vector{Float64}[]
masterˈs_algorithm()

f = Figure(); # once only

panel = f[2, 2]
ax2 = Axis(panel; yaxisposition = :right, yscale = log10, yticklabelcolor = :green); # for count of cuts
ax1 = Axis(panel; yscale = log10, yticklabelcolor = :blue); # for violation
barplot!(ax2, eachindex(pvec), map(length, pvec); color = :palegreen1);
scatter!(ax1, eachindex(pvec), map(mean, pvec); color = :royalblue1);
hidespines!(ax2);
